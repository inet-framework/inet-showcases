...
Block Acknowledgment is an 802.11 MAC feature, which increases throughput by decreasing
protocol overhead. Instead of individually acknowledging received packets, a single block
acknowledgment frame can acknowledge multiple packets. This reduces the number of acks
(and corresponding interframe spaces and contention periods).
...
Currently, INET only supports `normal block ack`. Normal block acks contain a bitmap,
...
There are several submodules of :ned:`Hcf` involved in the block ack mechanism.
These modules have parameters controlling the block ack behavior. The following
are some of the main parameters, the containing module types, and their locations
...
They have the same parameters. The parameters of :ned:`OriginatorBlockAckAgreementPolicy`
(at ``mac.hcf.originatorBlockAckAgreementPolicy``) and :ned:`RecipientBlockAckAgreementPolicy`
(at ``mac.hcf.recipientBlockAckAgreementPolicy``):
...
the originator module. In :ned:`OriginatorQosAckPolicy` (at ``mac.hcf.originatorAckPolicy``):
...
There are two hosts; ``host1`` sends UDP packets to ``host2``. There are three configurations,
demonstrating various aspects of the block ack mechanism. The configurations in :download:`omnetpp.ini <../omnetpp.ini>` are
the following:
...
in :download:`omnetpp.ini <../omnetpp.ini>`. The important ones are highlighted below:
...
The max block ack frame length is left on default, 1000B.
...
.. figure:: startingframesequence.png
   :width: 100%
   :align: center
...
It's indicated in the block ack when some of the frames in a block were not received correctly.
The MAC retransmits those in the next block. Here is a retransmission in the packet traffic view:

.. figure:: retransmission.png
   :width: 100%
   :align: center
...
are sent after 16 frames. It's defined in the ``Fragmentation`` configuration in :download:`omnetpp.ini <../omnetpp.ini>`:
...
.. figure:: frag_blockack_messageview.png
   :width: 100%
   :align: center

In the next block, two fragments of `Data-1` are retransmitted, then many fragments of `Data-2`
are sent:

.. figure:: frag_retx_messageview.png
   :width: 100%
   :align: center

Here is the block ack bitmap for this block:

.. figure:: frag_retx_ba.png
   :width: 90%
   :align: center
...
entry indicates that there were three lost fragments of `Data-2`.
