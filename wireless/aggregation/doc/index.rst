...
.. 802.11 frame aggregation is a feature in which multiple data packets can be sent
   in a single 802.11 transmission. Aggregation increases throughput by decreasing
   802.11 protocol overhead.

When sending small packets, 802.11 protocol overhead becomes significant, which hampers performance. Frame aggregation is a feature which can mitigate the performance hit, by sending multiple data packets in a single 802.11 transmission. The use of aggregation decreases the number of contention periods, interframe spaces and protocol headers, thus increasing throughput.
...
.. This showcase contains an example simulation which demonstrates that frame aggregation is useful when sending lots of small packets.
...
:ned:`BasicMsduAggregationPolicy` has a few parameters:
...
parameters (if it wouldn't comply with any of them, then no a-MSDU would be created,
...
aggregation (and also with and without using TXOP), and examine the number
of received packets, the throughput and the end-to-end delay.
...
One of the hosts, ``host1`` sends small, 100-byte UDP packets to ``host2``.
The traffic settings in the ``General`` configuration in :download:`omnetpp.ini <../omnetpp.ini>`:
...
The 802.11 settings in the ``General`` configuration in :download:`omnetpp.ini <../omnetpp.ini>`:
...
- ``Aggregation``: Aggregation is on, the MAC sends UDP packets in the best effort access category. Throughput is expected to increase substantially.
- ``VoicePriorityAggregation``: Aggregation is on, the MAC sends UDP packets in the voice access category in a TXOP. The use of TXOP reduces contention periods; throughput is expected to increase even further.
...
.. figure:: receivedpackets_t.png
   :width: 80%
   :align: center

.. figure:: avgthroughput_t.png
   :width: 90%
   :align: center
...
and a PHY header. The MAC contends for the channel before sending each packet. There is a SIFS period and an ACK after each packet transmission.
These amount to a lot of overhead.

In the ``Aggregation`` case, throughput increases by more than 400%, to about 11 Mbps.
...
There is also a SIFS period and an ACK after each packet transmission (though there are less of these compared to the previous case, as ten packets are acknowledged with an ACK).
...
.. Each data frame is followed by an ACK.
   In the case of the voice priority aggregation, the aggregate frames are shorter
   than in the best effort aggregation case, because the use of TXOP increases throughput,
   thus less packets accumulate in the queue.
...
If traffic is less than the maximum possible throughput, queues won't fill up (no increased delay), but the performance would be less than
the maximum possible. If traffic is the same as the maximum possible throughput, the performance
is optimal, and the delay is minimized (we configured traffic this way).

.. To negate the effect of queue length on the delay of packets, we set the amount of
   traffic to match the possible throughput. This way we can concentrate on the effect
   aggregation has on the delay. We run the three simulations again, but with the
   amount of traffic matching the maximum possible throughput. The configurations
   for these simulations are ``DelayNoAggregation``, ``DelayAggregation`` and
   ``DelayVoicePriorityAggregation``. Each extends the corresponding original
   configuration, and sets a different amount of traffic, by modifying the send
   interval of the UDP app.
