...
examine ``TransportRouteVisualizer`` that can provide graphical feedback
...
``TransportRouteVisualizer``. Transport path activity visualization is
...
``TransportRouteVisualizer`` follows packets throughout their path so
...
The wired network contains two connected ``StandardHost`` type nodes:
...
``TransportRouteVisualizer`` between the two hosts, indicating transport
...
continuously generating traffic by a ``UdpBasicApp`` application, which
is handled by a ``UdpSink`` application in the destination nodes
...
``TransportRouteVisualizer`` NED documentation.
