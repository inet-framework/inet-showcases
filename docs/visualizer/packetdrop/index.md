...
It contains two <var>StandardHosts</var>, connected with an ethernet cable. The ethernet cable is configured in the NED file to be disabled. Additionally, <var>host1</var> is configured to ping <var>host2</var>.

Since the cable between the hosts is configured to be disabled, the MAC module is unable to send the packets, and drops them. This is illustrated on the following animation:

<p><video autoplay loop controls onclick="this.paused ? this.play() : this.pause();" src="packetdrop22.mp4" width="414" height="477"></video></p>

Note the packet drop animation at <var>host1</var>. The packet drops are also evidenced in the log: 
